...
namespace SrcChess2.FICSInterface {
    ...
    public class FICSConnection : IDisposable {
        ...
        private void ProcessMoveListLine(string strLine) {
            bool?       bResult;
            string      strError;
            GameIntf    gameIntf;

            gameIntf    = m_state.CurrentGameIntf;
            bResult     = gameIntf.ParseInitialMove(strLine, out strError);
            if (bResult == null) {
                m_state.ResetCommand();
                if (!gameIntf.CreateInitialBoard(out strError)) {
                    m_state.TerminateGame(gameIntf, TerminationE.TerminatedWithErr, "", strError);
                }
            } else if (bResult == false) {
                m_state.TerminateGame(gameIntf, TerminationE.Terminated, "Error", null);
                m_state.ResetCommand(strError);
            }
        }
