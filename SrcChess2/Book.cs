...
namespace SrcChess2 {
    ...
    public class Book {
        ...
        /// </returns>
        private int CreateEntries(List<short[]> arrMoveList, List<BookEntry> arrBookEntry, List<short> arrKey, out int iPosIndex, int iDepth, PgnParser.delProgressCallBack callback, object cookie) {
            int         iRetVal = 0;
            int         iKeySize;
            short       nOldValue;
            ...
            if (iDepth != 0) {
                for (int iIndex = 0; iIndex < iRetVal; iIndex++) {
                    if (callback != null) {
                        callback(cookie, PgnParser.ParsingPhaseE.CreatingBook, 0, 0, null, iIndex, iRetVal);
                    }
                    arrKey.Add(arrValues[iIndex]);
                    entry                           = arrBookEntry[iPosIndex+iIndex];
                    entry.Index                     = iPosIndex;
                    entry.Size                      = (short)CreateEntries(arrMoveList, arrBookEntry, arrKey, out entry.Index, iDepth - 1, null, null);
                    arrBookEntry[iPosIndex+iIndex]  = entry;
                    arrKey.RemoveAt(arrKey.Count - 1);
                }
                if (callback != null) {
                    callback(cookie, PgnParser.ParsingPhaseE.CreatingBook, 0, 0, null, iRetVal, iRetVal);
                }
            }
            return(iRetVal);
        }
        ...
        /// Nb of entries created
        /// </returns>
        private BookEntry[] CreateBookList(List<short[]> arrMoveList, int iMaxDepth, PgnParser.delProgressCallBack callback, object cookie) {
            ...
            entry.Weight    = 0;
            arrBookEntry.Add(entry);
            entry.Size      = (short)CreateEntries(arrMoveList, arrBookEntry, arrKey, out entry.Index, iMaxDepth - 1, callback, cookie);
        ...
        /// Nb of entries created
        /// </returns>
        public int CreateBookList(List<short[]> listMoveList, int iMinMoveCount, int iMaxDepth, PgnParser.delProgressCallBack callback, object cookie) {
            short[]         pnLast = null;
            ...
            m_bookEntries = CreateBookList(arrUniqueMoveList, iMaxDepth, callback, cookie);
            ComputeWeight();
