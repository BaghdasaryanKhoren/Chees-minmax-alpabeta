...
namespace SrcChess2 {   
    ...
    public partial class ColorPicker : UserControl
    {
        ...
        void txt_TextChanged(object sender, RoutedEventArgs e)
        {
            int     iVal;
            Color   color;

            if (!string.IsNullOrEmpty(((TextBox)sender).Text)) {
                if (Int32.TryParse(((TextBox)sender).Text, out iVal)) {
                    if (iVal > 255) {
                        ((TextBox)sender).Text = "255";
                    } else {
                        if (MakeColorFromRGB(out color)) {
                            CustomColor = color;
                            Reposition();
                        }
                    }
        ...
        private bool MakeColorFromRGB(out Color color)
        {
            ...
            bRetVal =   Byte.TryParse(txtAlpha.Text, out abyteValue)    &&
        ...
        private void NumericValidation(System.Windows.Input.KeyEventArgs e)
        {
            ...
            if (e.Key == Key.Enter) {
                ...
                if (strInput.StartsWith("D")) {
                    strInput = strInput.Substring(1);
                } else if (strInput.StartsWith("NumPad")) {
                    strInput = strInput.Substring(6);
                }
                if (!Int32.TryParse(strInput, out iVal)) {
                    e.Handled = true; 
                }
            }
        }
